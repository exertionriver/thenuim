...
class DVLayoutHandler(var screenWidth: Float, var screenHeight: Float) : Telegraph {
    ...
    fun adjacencyPaneRows(currentFontSize: FontSize) : MutableMap<Int, Int?> = mutableMapOf()
    fun adjacencyTopPadOffset(currentFontSize: FontSize) : MutableMap<Int, Int?> = mutableMapOf()
    ...
    fun imagePanes() : List<Int> = definePanes().data.keys.toList()
    fun textPanes() : List<Int> = listOf()

    fun paneTable(DVLayout: DVLayout) : Table {

        val layoutTable = Table()

        DVLayout.layout.firstOrNull{ it.tableIdx == "layout" }?.panes?.forEach { pane ->
     //       layoutTable.add(it)
        }
    ...
    fun paneCell(DVLayout: DVLayout, displayViewLayoutCell: DVLayoutCell) {

        when  {
            (displayViewLayoutCell is DVPane) -> {}
            (displayViewLayoutCell is DVRow) -> {}
            (displayViewLayoutCell is DVTable) -> {}
            else -> {}
        }
