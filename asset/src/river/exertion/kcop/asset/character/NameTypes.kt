package river.exertion.kcop.asset.character

import java.util.*

enum class NameTypes {
    CELESTIAL_OSH { override fun nextName() = NameGenerator.innerShuffle(
        NameGenerator.filterOutSyllableShuffle(0)
    )
    },
    CELESTIAL_UR { override fun nextName() = NameGenerator.innerShuffle(
        NameGenerator.filterOutSyllableShuffle(1)
    )
    },
    CELESTIAL_LA { override fun nextName() = NameGenerator.innerShuffle(
        NameGenerator.filterOutSyllableShuffle(2)
    )
    },
    CELESTIAL_DI { override fun nextName() = NameGenerator.innerShuffle(
        NameGenerator.filterOutSyllableShuffle(3)
    )
    },
    CELESTIAL_SE { override fun nextName() = NameGenerator.innerShuffle(
        NameGenerator.filterOutSyllableShuffle(4)
    )
    },
