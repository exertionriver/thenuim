...
object NarrativeLoPa : IDisplayViewLoPa, IMessagingLoPa, IAssetLoPa, IECSLoPa, IMenuLoPa {
    ...
    private fun addNarrativeInfoToMainMenu() {
        ...
        MainMenu.menuPane = {
            mainMenuPane().apply {
                ...
                this.add(Label("Narrative:", TnmSkin.labelStyle(TnmFont.MEDIUM, NarrativeMenuText))).left()
                this.row()

                if (isNarrativeLoaded() && currentNarrativeAsset.assetInfo().isNotEmpty()) {
                    currentNarrativeAsset.assetInfo().forEach { narrativeEntry ->
                        this.add(
                            Label(narrativeEntry, TnmSkin.labelStyle(TnmFont.SMALL, NarrativeMenuText))
                                .apply {
                                    this.wrap = true
                                }).growX().left()
                        this.row()
                    ...
                    this.add(
                        Label(NoNarrativeLoaded, TnmSkin.labelStyle(TnmFont.SMALL, NarrativeMenuText))
                    ).growX().left()
                }
